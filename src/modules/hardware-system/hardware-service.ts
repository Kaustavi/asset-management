...
export async function hardwareGetById(req: Request, res: Response) {
  try {
    ...
    const device = await prismaClient.hardwareSystem.findFirst({ where: { id: deviceId } });
    return res.status(200).json(device);
  } catch (err: any) {
    console.log(err);
    return res.json({ message: err.message });
...
export async function hardwareDeleteById(req: Request, res: Response) {
  try {
    ...
    const deletedDevice = await prismaClient.hardwareSystem.delete({ where: { id: deviceId } });
